...
const renderFunctionForm = (contractCard, contractMeta, fn) => {
  ...
  header.innerHTML = `
    <div>
      <h4>${fn.name}</h4>
      <p>${fn.inputs.map(input => `${input.type} ${input.name || ''}`.trim()).join(', ') || 'No inputs'}</p>
    </div>
    <span class="badge ${isRead ? 'badge-read' : 'badge-write'}">${isRead ? 'Read' : 'Write'}</span>
  `
  ...
  if (!isRead && fn.stateMutability === 'payable') {
    valueInput = document.createElement('label')
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      const address = contractCard.querySelector('input[name="address"]').value.trim()
      ...
      if (isRead) {
        ...
        if (fn.stateMutability === 'payable') {
          ...
          if (valueField?.value) {
            overrides.value = ethers.parseEther(valueField.value)
          }
        }
  ...
  card.innerHTML = `
    <header>
      <div>
        ...
        <p class="muted">Address: <input name="address" value="${contractMeta.address}" /></p>
      </div>
    </header>
  `
...
const renderContracts = () => {
  ...
  contractConfigs.forEach(contract => {
    contractsRoot.appendChild(renderContractCard(contract))
  })
}
