...
if (!ethers) {
  throw new Error('Ethers library not loaded. Ensure the ethers script is included before bondToken.js.');
}
...
function normalizeAbi(abiData) {
  ...
  throw new Error('Invalid Bond Token ABI format.');
}

async function getBondTokenAbi() {
  if (bondTokenAbi) {
    return bondTokenAbi;
  }
  const embedded = document.getElementById('bond-token-abi');
  if (embedded?.textContent?.trim()) {
    bondTokenAbi = normalizeAbi(JSON.parse(embedded.textContent));
    return bondTokenAbi;
  }
  const response = await fetch(BOND_TOKEN_ABI_URL);
  if (!response.ok) {
    throw new Error(`Failed to load Bond Token ABI (${response.status})`);
  }
  bondTokenAbi = normalizeAbi(await response.json());
  return bondTokenAbi;
}
...
function parseTokenAmount(value) {
  ...
  if (tokenDecimals === 0 && !Number.isInteger(parsed)) {
    throw new Error('Amount must be a whole number for bonds.');
  }
...
async function ensureBondToken() {
  if (bondToken) {
    return bondToken;
  }
  ...
  bondToken = new ethers.Contract(BOND_TOKEN_ADDRESS, abi, signer);
  tokenDecimals = Number(await bondToken.decimals());
  return bondToken;
}
...
function renderContractAddress() {
  ...
  const explorerLink = `${EXPLORER_BASE}/address/${BOND_TOKEN_ADDRESS}`;
  target.innerHTML = `Contract: <a href="${explorerLink}" target="_blank">${BOND_TOKEN_ADDRESS}</a>`;
}
